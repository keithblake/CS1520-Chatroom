...
def home():
	...
	if request.method == 'POST':
		if not request.form['name']:
			...
			db.session.commit()
			cr = Chatroom.query.all()
			return render_template('login.html', error=error, rooms=cr)
	return render_template('index.html', error=error, rooms=cr)
...
def create_chatroom():
	...
	db.session.add(Chatroom(name))
	db.session.commit()
	cr = Chatroom.query.filter_by(name = name).first()
	return redirect(url_for('chatroom', id=cr.chatroom_id))
...
def chatroom(id):
	...
	if request.method == 'POST':
		if not request.form['msgs']:
			...
			return render_template('chatroom', id=id, msgs=messages, error=error) 
